...
- Current Rust status:
  ...
  - WebSocket opt-in capability `agent-events-v1` now enables server-push `evt` frames for agent lifecycle/assistant updates and chat final/error updates, scoped by run origin connection metadata.
...
- Current Rust status:
  ...
  - Slack HTTP compatibility includes `POST /slack/events` (and configurable `slackEventsPath`) mapped to the Slack webhook adapter.
  - Additional tool types and full Slack HTTP surface parity are not yet implemented.
